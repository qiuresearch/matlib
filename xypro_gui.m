      ...
      switch hObject
        ...
        case hdatapro.gui2selectdata
            ...
            if (iselect(1) ~=0)
               ...
               gui_fields = {gui_fields{:}, 'ifunc', 'funclist', ...
                            'algorithmlist', 'fitpar'}'
               % note: values are copied from the first selected
               % dataset, NOT GUI!!!
               gui_values = cell2struct(num2cell(ones(numel(gui_fields),1)), ...
                                      gui_fields,1);
               gui_values = struct_assign(xc.xydata(iselect(1)), ...
                                        gui_values, 'append', 0);
               for i=1:length(iselect);
                  xc.xydata(iselect(i)) = struct_assign(gui_values, ...
