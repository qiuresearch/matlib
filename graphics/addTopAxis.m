function [newAxis] = addTopAxis (TopTickPositions,TopTickLabels,AxisName)
...
%
%  Modified by Xiangyun (10/22/2013)
%    1) clean up the code a bit
%    2) set box off for new axis as well to avoid double x xticks
%    at the bottom
    ...
    set(newAxis, 'xaxisLocation', 'top', 'YAxisLocation', 'right');
    set(oldAxis, 'xaxislocation', 'bottom', 'YAxisLocation', 'left');
    ...
    set(oldAxis, 'box', 'off');
    % remove grids
    set(newAxis, 'box', 'off', 'xgrid', 'off', 'yGrid', 'off');

    set(newAxis, 'YScale', get(oldAxis, 'YScale'));
    set(newAxis, 'YTick', get(oldAxis, 'YTick'), 'yTickLabel', []);
    ...
    hlink = linkprop([newAxis, oldAxis], {'Position','YTick','YMinorTick','XScale','YScale'});
