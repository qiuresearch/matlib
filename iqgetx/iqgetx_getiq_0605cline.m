function [iq, sIqgetx]=iqgetx_getiq_0605cline(sIqgetx, startnum, skipnums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: iqgetx_getiq_0605cline.m,v 1.2 2013/08/19 03:02:25 xqiu Exp $
%

verbose = 1;
% 1) Simple check on input parameters
if nargin < 1
   help iqgetx_getiq_0605cline
   return
end
...
for i=1:num_data
   ...
   for datatype=1:3
       ...
       % ----- The difference in 0605cline here!!!!! ------
       
       [sumdata, imgdata] = slurp_sg1kccd_0605cline(sIqgetx(i).prefix, ...
                    (imgnums(1)+2):(imgnums(1)+imgnums(2)+1), ...
                    [imgnums(1),imgnums(1)+1], sIqgetx(i));

       for iimg=2:length(imgnums)/2
          [sumdatatmp, imgdatatmp]=slurp_sg1kccd_0605cline(sIqgetx(i).prefix, ...
                    (imgnums(2*iimg-1)+2):(imgnums(2*iimg-1)+imgnums(2*iimg)+1), ...
                    [imgnums(2*iimg-1),imgnums(2*iimg-1)+1], sIqgetx(i));
