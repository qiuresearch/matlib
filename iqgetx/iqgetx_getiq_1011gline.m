function [iq, sIqgetx] = iqgetx_getiq_1011gline(sIqgetx, startnum, skipnums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: iqgetx_getiq_1011gline.m,v 1.3 2013/08/23 18:52:47 xqiu Exp $
%
...
for i=1:num_data
   ...
   switch sIqgetx(i).dataformat
      case 'SG1KCCD'
         slurp_saxsdata = @slurp_sg1kccd_0705cline;
      case 'FLICAM'
         slurp_saxsdata = @slurp_flicam_1011gline;
      case 'MarCCD'
         slurp_saxsdata = @slurp_marccd_0802nsls;
      case 'ADSC'
         slurp_saxsdata = @slurpadsc_xq;           
      otherwise
   end
   ...
   if ~isempty(sIqgetx(i).darknums) && (sIqgetx(i).darksub == 1)
      ...
      darkdata = feval(slurp_saxsdata, sIqgetx(i).samprefix, ...
   ...
   for datatype=1:3
      ...
      if (sIqgetx(i).readraw == 0) % read corrected scans
         [sumdata, imgdata] = feval(slurp_saxsdata, prefix, imgnums, ...
                                    [], sIqgetx(i));
      else % have to read each scan one by one
         [sumdata, imgdata] = feval(slurp_saxsdata, prefix, imgnums(1), ...
                                    [], sIqgetx(i));
         for iimg=2:length(imgnums)
            [sumdatatmp, imgdatatmp] = feval(slurp_saxsdata, prefix, ...
