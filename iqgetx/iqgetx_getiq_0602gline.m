function [iq, sIqgetx]=iqgetx_getiq_2buf_0602gline(sIqgetx, startnum, skipnums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: iqgetx_getiq_0602gline.m,v 1.3 2013/08/19 03:02:25 xqiu Exp $
%

verbose = 1;
% 1) Simple check on input parameters
if nargin < 1
   help iqgetx_getiq_0602gline
   return
end
...
for i=1:num_data
   ...
   for datatype=1:3
       ...
       if (sIqgetx(i).readraw == 0)
          [sumdata, imgdata] = slurp_flicam_0602gline(sIqgetx(i).prefix, ...
                                                      imgnums, [], sIqgetx(i));
       else
          [sumdata, imgdata] = slurp_flicam_0602gline(sIqgetx(i).prefix, ...
                                               imgnums(1), imgnums(1)-1, ...
                                               sIqgetx(i));
          for iimg=2:length(imgnums)
             [sumdatatmp, imgdatatmp] = ...
                 slurp_flicam_0602gline(sIqgetx(i).prefix, imgnums(iimg), ...
                                        imgnums(iimg)-1, sIqgetx(i));
