function [sumdata, imgdata] = slurp_flicam_0704gline(prefix, scannum, darknums, sOpt)
% --- Usage:
%        [sumdata, imgdata] = slurpadsc_flicam_0610gline(prefix,
...
%
%           3) if "readraw == 1", only one scannum is
...
% --- Example(s):
%
% $Id: slurp_flicam_0704gline.m,v 1.3 2013/08/19 03:02:26 xqiu Exp $

verbose = 1;
if nargin < 1
   error('at least one parameter is required, please see the usage! ')
   help slurq_flicam_0704gline
   return
end
...
if nargin < 4
   ...
   sOpt.setbkgzero = 1;   %
   sOpt.normalize = 1;
   sOpt.readraw = 1;   % whether to read the raw images or corrected
   sOpt.datadir = '';
   sOpt.suffix = '.tif';
   sOpt.im_dark = 0;
   sOpt.normconst_dark = 0;
end
...
if (nargin > 2) && ~isempty(darknums) % if darknums is passed, use it!
   ...
   darkdata = slurp_flicam_0704gline(prefix, darknums, [], sOpt2);
...
if ~exist('scannum', 'var') || isempty(scannum)
   ...
   if (sOpt.readraw == 1) % read every raw file (assume only one
                          % scan to read!!!)
      prefix2 = [prefix sprintf('%03i_', scannum)];
...
if isfield(imgdata(1), 'moncounts')
   ...
   sumdata.moncounts_dark = sOpt.moncounts_dark;
   sumdata.normconst_dark = sOpt.normconst_dark;
end
...
if isfield(sOpt, 'lblcorrect') && (sOpt.lblcorrect == 1)
   ...
   sumdata.im(:,:) = lblcorrect_flicam_0704gline(sumdata.im, 'setbkgzero', sOpt.setbkgzero);
   for i=1:length(imgdata)
      imgdata(i).im = lblcorrect_flicam_0704gline(imgdata(i).im, 'setbkgzero', ...
                                        sOpt.setbkgzero);
