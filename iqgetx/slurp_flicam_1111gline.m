function [sumdata, imgdata] = slurp_flicam_1111gline(prefix, scannum, darknums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: slurp_flicam_1111gline.m,v 1.6 2013/08/19 03:02:26 xqiu Exp $
...
if ~exist('scannum', 'var') || isempty(scannum)
   ...
   if ~isempty(specdata)
      ...
      if isempty(iscan)
         ...
         monnames = {'gdoor', 'diode', 'i1', 'epoch', 'time', 'mean'};
...
if isfield(sOpt, 'lblcorrect') && (sOpt.lblcorrect == 1)
   showinfo(['doing line by line flicam correction, set background to zero:' ...
    ...
    [sumdata.im(:,:), row_x] = imcorr_linebyline(sumdata.im, sOpt.covered_columns, ...
                                       'setbkgzero', sOpt.setbkgzero);
     figure(1); hold all; hplot=plot(row_x); legend_add(hplot, 'averaged image');

   for i=1:length(imgdata)
     [imgdata(i).im, row_x] = imcorr_linebyline(imgdata(i).im, ...
