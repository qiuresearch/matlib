function [sumdata, imgdata] = slurp_flicam_0610gline(prefix, scannums, darknums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: slurp_flicam_0610gline.m,v 1.3 2013/08/19 03:02:25 xqiu Exp $

verbose = 1;
if nargin < 1
   error('at least one parameter is required, please see the usage! ')
   help slurq_flicam_0610gline
   return
end
...
if (nargin > 2) && ~isempty(darknums) % if darknums is passed, use it!
   ...
   darkdata = slurp_flicam_0610gline(prefix, darknums, [], sOpt2);
...
if ~exist('scannums', 'var') || isempty(scannums)
   ...
   if (sOpt.readraw == 1) % read every raw file (assume only one
      ...
      for i=1:num_imgs
         ...
         imgdata(i).num = scannums + signums(i)/num_imgs;
...
if isfield(sOpt, 'lblcorrect') && (sOpt.lblcorrect == 1)
   ...
   sumdata.im(:,:) = lblcorrect_flicam(sumdata.im, 'setbkgzero', sOpt.setbkgzero);
   for i=1:length(imgdata)
      imgdata(i).im = lblcorrect_flicam(imgdata(i).im, 'setbkgzero', ...
                                        sOpt.setbkgzero);
