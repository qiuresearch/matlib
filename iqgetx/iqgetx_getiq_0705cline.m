function [iq, sIqgetx]=iqgetx_getiq_0705cline(sIqgetx, startnum, skipnums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: iqgetx_getiq_0705cline.m,v 1.4 2013/08/23 18:52:47 xqiu Exp $
%

verbose = 1;
% 1) Simple check on input parameters
if nargin < 1
   help iqgetx_getiq_0705cline
   return
end
...
for i=1:num_data
   ...
   for datatype=1:3
       ...
       [sumdata, imgdata] = slurp_sg1kccd_0705cline(sIqgetx(i).prefix, ...
                    (imgnums(1)+2):(imgnums(1)+imgnums(2)+1), ...
                    [imgnums(1),imgnums(1)+1], sIqgetx(i));

       for iimg=2:length(imgnums)/2
          [sumdatatmp, imgdatatmp]=slurp_sg1kccd_0705cline(sIqgetx(i).prefix, ...
