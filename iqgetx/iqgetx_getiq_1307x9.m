...
% --- Example(s):
%
% $Id: iqgetx_getiq_1307x9.m,v 1.2 2013/08/23 18:52:47 xqiu Exp $
%
...
for i=1:num_data
   ...
   for datatype=1:3
      ...
      % ----- The difference in ????? here!!!!! ------
       ...
       switch sIqgetx(i).dataformat % read in the data
           case 'pilatus'
              [sumdata, imgdata] = slurp_pilatus_1307x9(prefix, ...
                                                    imgnums, [], sIqgetx(i));
            otherwise
       end
       ...
       if (sumdata.aniso_calc == 1);
          ...
          if isempty(sumdata.aniso_qpos) % if not yet, use peak position
          ...
          [sumdata.aniso_prof, sumdata.aniso_circ] = ...
              circ_integrate(sumdata.im, sumdata.aniso_qpos, ...
                             sumdata.aniso_qwid);
   ...
   if ~isempty(sIqgetx(i).buf)
      ...
      sIqgetx(i).sambuf_scale = scale*sIqgetx(i).sambuf_scale;
