function [iq, sIqgetx] = iqgetx_getiq_1002x6b(sIqgetx, startnum, skipnums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: iqgetx_getiq_1002x6b.m,v 1.3 2013/08/23 18:52:47 xqiu Exp $
%
   ...
   for datatype=1:3
      ...
      % ----- The difference in ????? here!!!!! ------
       ...
       switch sIqgetx(i).dataformat % read in the data
           case 'SPE'
              [sumdata, imgdata] = slurp_spe_1002x6b(prefix, imgnums, ...
                                                     [], sIqgetx(i));
           case 'SG1KCCD'
       ...
       % get I(q) -- integration
       sumdata.iq= integrater(sumdata.im, 'pixel', sumdata.smooth);
       for k=1:length(imgdata) % individual ones are not dezingered
           imgdata(k).iq = integrater(imgdata(k).im, 'pixel', imgdata(k).smooth);
       end
