function [iq, sIqgetx]=iqgetx_getiq_0704gline(sIqgetx, startnum, skipnums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: iqgetx_getiq_0704gline.m,v 1.3 2013/08/23 18:52:47 xqiu Exp $
%

verbose = 1;
% 1) Simple check on input parameters
if nargin < 1
   help iqgetx_getiq_0704gline
   return
end
...
for i=1:num_data
       ...
       switch sIqgetx(i).dataformat
           case 'SG1KCCD'
              darkdata = slurp_sg1kccd_xq(sIqgetx(i).prefix, ...
                                          ...
                                          sIqgetxtmp);
          case 'FLICAM'
             darkdata = slurp_flicam_0704gline(sIqgetx(i).prefix, ...
                                               sIqgetx(i).darknums, [], ...
                                               sIqgetxtmp);
          otherwise
       end
   ...
   for datatype=1:3
       ...
       % ----- The difference in 0602gline here!!!!! ------
       
       if (sIqgetx(i).readraw == 0) % read corrected scans
          [sumdata, imgdata] = slurp_flicam_0704gline(sIqgetx(i).prefix, ...
                                                      imgnums, [], sIqgetx(i));
          ...
          for iimg=2:length(imgnums)
             [sumdatatmp, imgdatatmp] = ...
                 slurp_flicam_0704gline(sIqgetx(i).prefix, imgnums(iimg), ...
                                        [], sIqgetx(i));
   ...
   % remove the dark image 2D array
   if (sIqgetx(i).keep_im == 0) 
      sIqgetx(i).im_dark = 0;
   end
