function [sumdata, imgdata] = slurp_sg1kccd_0505cline(prefix, imgnums, darknums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: slurp_sg1kccd_0505cline.m,v 1.3 2013/08/19 03:02:26 xqiu Exp $
%
...
if (nargin > 2) && ~isempty(darknums) % if darknum is passed, use it!
   ...
   sOpt.im_dark = 0;
   darkdata = slurp_sg1kccd_xq(prefix, darknums, [], sOpt);
...
for i=1:num_imgs
   ...
   imgdata(i).monnames = {'xflash', 'i1', 'i0', 'cesr'};
   imgdata(i).moncounts = tags([8,7,6,5]);
   if (sOpt.normalize ~= 0)
...
% --- Change History:
%
% $Log: slurp_sg1kccd_0505cline.m,v $
