function [sumdata, imgdata] = slurp_marccd_0802nsls(prefix, imgnums, darknums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: slurp_marccd_0802nsls.m,v 1.4 2013/08/19 03:02:26 xqiu Exp $
%
...
if (nargin > 2) && ~isempty(darknums) % if darknum is passed, use it!
   ...
   sOpt.im_dark = 0;
   darkdata = slurp_marccd_0802nsls(prefix, darknums, [], sOpt);
...
for i=1:num_imgs
   ...
   % normalization: transmitted beam is from the beam stop
   imgdata(i).monnames = {'transbeam', 'bicron', 'bs', 'IC'};
   ...
   beamstop = [667, 711; 685, 759];
   transtop = [669, 716; 681, 726];
   ...
   % get "bicron", intensity
   if isfield(sOpt, 'specfile')
   ...
   imgdata(i).moncounts = [transbeam, bicron,1,1];
   if (sOpt.normalize ~= 0)
...
% --- Change History:
%
% $Log: slurp_marccd_0802nsls.m,v $
% Revision 1.4  2013/08/19 03:02:26  xqiu
% *** empty log message ***
%
% Revision 1.3  2012/02/07 00:09:52  xqiu
% *** empty log message ***
%
% Revision 1.2  2008-07-03 18:36:41  xqiu
% after the 0806NSLS trip committing
%
% Revision 1.1  2008-03-04 16:44:08  xqiu
% afer 0802NSLS x21 run
%
