function [sumdata, imgdata] = slurp_pilatus_1307x9(prefix, scannum, darknums, sOpt)
% --- Usage:
...
% --- Example(s):
%
% $Id: slurp_pilatus_1307x9.m,v 1.2 2013/08/19 03:02:26 xqiu Exp $
...
if (nargin < 4) || isempty(sOpt)
   ...
   sOpt.readraw = 1;    % read the raw images (1) or corrected (0)
   sOpt.datadir = '';
   sOpt.suffix = '';
   sOpt.im_dark = 0;  % Arthur used 100 as the pedestal
...
templ_struct = struct('prefix', '', 'num', 0, 'suffix', sOpt.suffix, ...
                      'file', '', 'im', 0.0, 'time', 0, 'mean', 0, 'monnames', monnames, 'moncounts', 1, 'normconst_dark', 0, 'moncounts_dark', 0);
...
if ~exist('scannum', 'var') || isempty(scannum)
   ...
   if (sOpt.readraw == 1) % read every raw file (assume only one
      ...
      for iscan = 1:length(scannum)
         prefix2 = sprintf('x%03i-%s-', scannum(iscan), prefix);
         num_imgs = length(dir([sOpt.datadir prefix2 '*' suffix]));
         signums = 1:num_imgs;
         newimgdata = repmat(templ_struct, 1, num_imgs);
         for i=1:num_imgs
            newimgdata(i).prefix = prefix2;
            newimgdata(i).num = scannum(iscan) + signums(i)/num_imgs;
            newimgdata(i).file = [sOpt.datadir newimgdata(i).prefix ...
                               sprintf('%i', signums(i)) suffix];
         end
         imgdata = [imgdata, newimgdata];
      end
   end
   ...
   if exist('specdata', 'var') && ~isempty(specdata)
      ...
      if isempty(iscan)
         ...
         monnames = {'gdoor', 'diode', 'i1', 'epoch', 'time'};
         igdoor = strmatch('GDOOR', upper(specdata(iscan).columnnames), 'exact');
         idiode = strmatch('DIODE',  upper(specdata(iscan).columnnames), 'exact');
...
for i=1:length(imgdata)
   if exist(imgdata(i).file, 'file'); 
      ...
      showinfo(['reading image: ' imgdata(i).file num2str(size(imgdata(i).im), ', dim:%ix%i') ', avg: ' num2str(imgdata(i).mean)]);
   else
   ...
   if ~isempty(specdata) && ~isempty(iscan)
      ...
      if (sOpt.readraw == 1) % images are all in one scan
         ...
         if ~isempty(iscan)
            ...
            imgdata(i).moncounts = [specdata(iscan).data(i,igdoor), ...
                                specdata(iscan).data(i,idiode), ...
                                specdata(iscan).data(i,ii1), ...
                                specdata(iscan).data(i,iepoch), ...
                                specdata(iscan).data(i,itime), ...
                                imgdata(i).mean];
         else
      ...
      if (sOpt.readraw == 0) % all counts in one scan should be summed
         ...
         if ~isempty(iscan)
            ...
            imgdata(i).moncounts = [sum(specdata(iscan).data(:,igdoor)), ...
                                sum(specdata(iscan).data(:,idiode)), ...
                                sum(specdata(iscan).data(:,ii1)), ...
                                sum(specdata(iscan).data(:,iepoch)), ...
                                sum(specdata(iscan).data(:,itime)), ...
                                imgdata(i).mean];
         end
      end
      ...
      if isfield(sOpt, 'normconst_dark')
         ...
         % from 224-237
         imgdata(i).normconst_dark =12150/10*imgdata(i).expotime;
      end
...
sumdata.aniso_calc = sOpt.aniso_calc;
