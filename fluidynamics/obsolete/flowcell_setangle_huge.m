function [fem, geoconst] = flowcell_setangle_huge(varargin)
...
fem.draw=struct('s',s);
[fem.geom, index, index2]=geomcsg(fem, 'out', {'g','st','ctx'} );
class(index), size(index), index(:,:)
index2{:}
...
fem.mesh=meshinit(fem, 'hmax',[init_size*4],  'hgradedg',[5,1.2,15, ...
                    1.2,18,1.2], 'hmaxedg',[6, init_size/10,9, ...
                    init_size/10,10, init_size/10,  11, init_size/10, ...
                    15,init_size/6,18, init_size/3], 'hmaxsub',[3, ...
...
bnd.p0 = 0;
bnd.ind = [3,4,1,1,5,1,2,1,2,4,1,1,1,2,4,1,2,4,1,6];
appl.bnd = bnd;
clear equ
equ.shape = [1;2;3];
equ.gporder = {{1;1;2}};
...
%bnd.type = {'Nc','Nc','C','C','N0'};
bnd.type = {'C','C','N0','cont','Nc'};
bnd.ind = [1,3,3,3,2,3,4,3,4,3,3,3,3,4,3,3,4,3,3,5];
appl.bnd = bnd;
clear equ
equ.shape = 1;
equ.gporder = 1;
