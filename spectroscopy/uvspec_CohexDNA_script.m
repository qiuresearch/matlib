...
FileSuffix1 = dnasuffix;
FileFormat1 = dnaformat;
FileSuffix2 = cohexsuffix;
FileFormat2 = cohexformat;
...
for i = 1:length(samnames)
   ...
   data = uvspec_subbaseline(uvspec_readspectrum([dnadir dnafiles{i} ...
                       dnasuffix], 'dataformat', dnaformat), [320,360]);
   ...
   data = uvspec_subbaseline(uvspec_readspectrum([cohexdir cohexfiles{i} ...
                       cohexsuffix], 'dataformat', cohexformat), [700,720]);
   ...
   if ~exist('cohex2dna_dilution', 'var') || isempty(cohex2dna_dilution)
      cohex2dna_dilution = 10;
   end
   ODdna = dnadata.(samnames{i}).ODmax(2)*50/310*cohex2dna_dilution;
   ODdna_baseline =  dnadata.(samnames{i}).baseline*50/310*cohex2dna_dilution;
   ODcohex = cohexdata.(samnames{i}).ODmax(2)/0.056;
   ODcohex_baseline =  cohexdata.(samnames{i}).baseline/0.056;
   ...
   samnotes{i} = [cohexdata.(samnames{i}).title '-' samnames{i}];
end
% convert 
...
specdata_savefile(spec_stru, [savename '_ionnum'  num2str(iCohexMethod) ...
                    '.dat']);
...
legend(ha,samnotes, 'fontsize', 8, 'Interpreter', 'None');
legend boxoff
uvspec_label;
title(sprintf('CoHex (iCohexMethod=%1i:%s)', iCohexMethod, sCohexMethod{iCohexMethod}));
...
saveps(gcf, [savename '_ionnum' num2str(iCohexMethod) '.eps']);
