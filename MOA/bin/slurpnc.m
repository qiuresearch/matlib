% function result = slurpnc(prefix,signal, background)
% 			prefix = starting part of the image file name.
% 			signal = vector of image numbers
%  		background = vector of image backgrounds
%			result = final image.
%
% You MUST be in the same directory as the file you wish to look at.
% Merely adding the path will not work because the compiled C code
% that this function runs cannot access your path.
%
...
%        (now calls ctake2, which is equivalent C code)
%      uses .imm files and not .tif
%
...
if (nargin>2)
   ...
   im_backr = ctake2(strcat(prefix, bkgdstring{1},'.imm'));
   for k=2:length(background)
      %im_backr(:,:,k) = imread(sprintf('%s%d.tif',prefix,background(k)),'tif');
      im_backr(:,:,k) = ctake2(strcat(prefix, bkgdstring{k},'.imm'));
   end
