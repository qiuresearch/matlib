		 ...
		 int nrhs, const mxArray *prhs[] )
{
  ...
  double *Num_Pixels; double *Delta_Int;
  double r_min, r_max;
  ...
  if (nlhs!=4)
    {mexErrMsgTxt("Need three outputs");}
  
  /*  mexPrintf("Inside radial_integral.mex.c, done checking
      inputs/outputs \n"); */
  ...
  plhs[2] = mxCreateDoubleMatrix(1,Num_Bins,mxREAL);
  Num_Pixels=mxGetPr(plhs[2]);
  plhs[3] = mxCreateDoubleMatrix(1,Num_Bins,mxREAL);
  Delta_Int=mxGetPr(plhs[3]);
  
  /* STEP NINE -  Call radial_integral */
  /*radial_integral(image,mask,width,height,xcen,ycen,*/
  /*  q, Intensity, r_min, r_max, Num_Bins, mode); */
  radial_integral(image,mask,width,height,xcen,ycen,
		  q, Intensity, Num_Pixels, Delta_Int, r_min, r_max, Num_Bins, mode);
