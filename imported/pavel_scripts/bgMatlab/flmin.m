function [Xmin, Ymin] = flmin(x, y, low, high, dim, nspfit)
%FLMIN       Find local maximum.
%  [XMIN,YMIN] = FLMIN(X, Y, LOW, HIGH)  returns the position and value
...
%
%  [XMIN,YMIN] = FLMIN(X, Y, LOW, HIGH, DIM)  operates along the dimension
...
%  $Id: flmin.m 32 2007-03-12 21:49:36Z juhas $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[Ymin,idxIdx] = min(y(ndidx{:}), [], dim);
% make sure Xmin has the same size as Ymin
Xmin = zeros(size(idxIdx));
Xmin(:) = x(idx(idxIdx));
