...
%FOREACH   should work similarly as tcsh foreach...
%  FOREACH VAR NAME COMMANDS
%  FOREACH file *.m disp($file)
%  $VAR tries to quote the VAR string in COMMANDS when necessary
%  $$VAR never quotes
%  if NAME starts with &, contents of caller variable NAME is used
%  modifiers:  #file 	index of the file
%              $file:r 	file without the extension
%EXAMPLES:
%  FOREACH FILE &FILELIST 'DISP $FILE'
%  FOREACH FILE C:\AUTO* 'DISP $FILE'
%  FOREACH FILE *.M 'HELP $FILE'
%  FOREACH FILE *.M 'EDIT $FILE'

%if these characters are befor $VAR, $VAR is quoted
quotecharspre=',([''=';
if nargin<2
